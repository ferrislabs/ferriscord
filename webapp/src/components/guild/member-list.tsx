...
function toPresenceStatus(apiStatus: string): PresenceStatus {
  ...
  if (apiStatus === 'online' || apiStatus === 'idle' || apiStatus === 'offline')
    return apiStatus
  return 'offline'
}
...
export function MemberList({ guildId, className }: MemberListProps) {
  ...
  const membersWithPresence = members.map((m) => ({
    ...m,
    presenceStatus: (userPresences[m.user_id] ??
      toPresenceStatus(m.status)) as PresenceStatus,
  }))

  const grouped = GROUP_ORDER.reduce<
    Record<PresenceStatus, typeof membersWithPresence>
  >(
    (acc, status) => {
      acc[status] = membersWithPresence.filter(
        (m) => m.presenceStatus === status,
      )
      return acc
    },
  ...
  return (
    <div
      className={cn(
        'w-60 shrink-0 border-l border-sidebar-border bg-background flex flex-col',
        className,
      )}
    >
