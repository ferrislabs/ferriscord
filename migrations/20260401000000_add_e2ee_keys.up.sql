...
-- Signed pre-keys (X25519, rotated monthly)
...
CREATE INDEX idx_signed_prekeys_user ON user_signed_prekeys(user_id);

-- One-time pre-keys (X25519, consumed once)
...
CREATE INDEX idx_otp_unconsumed ON user_onetime_prekeys(user_id, consumed) WHERE NOT consumed;
...
-- Sender keys for guild channels
CREATE TABLE channel_sender_keys (
    ...
    created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
    UNIQUE(channel_id, sender_user_id, generation)
);
...
-- DM sessions (Double Ratchet state, encrypted per device)
